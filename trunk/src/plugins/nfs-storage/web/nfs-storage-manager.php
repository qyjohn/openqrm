			...
			switch (htmlobject_request('action')) {
				case 'select':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $id) {
							...
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/nfs-storage/bin/openqrm-nfs-storage post_exports -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				...
				case 'add':
					if (strlen($nfs_storage_id)) {
						...
						$storage_resource = new resource();
						$storage_resource->get_instance_by_id($storage->resource_id);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/nfs-storage/bin/openqrm-nfs-storage add -n $nfs_lun_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$storage_resource_id = $storage_resource->id;
						$statfile="storage/".$storage_resource_id.".nfs.stat";
						$statfile_manual="storage/".$storage_resource_id.".nfs.stat.manual";
						...
						if (isset($_REQUEST['identifier'])) {
							...
							if (file_exists($statfile_manual)) {
								...
								foreach($_REQUEST['identifier'] as $nfs_lun_name) {
									$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/nfs-storage/bin/openqrm-nfs-storage remove -n $nfs_lun_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
									$storage_resource->send_command($storage_resource->ip, $resource_command);
				...
				case 'snap':
					if (strlen($nfs_storage_id)) {
						...
						if (file_exists($statfile_manual)) {
							...
							// send command
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/nfs-storage/bin/openqrm-nfs-storage snap -n $nfs_lun_name -s $nfs_lun_snap_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
							$storage_resource->send_command($storage_resource->ip, $resource_command);
							// and wait for the resulting statfile
							if (!wait_for_statfile($statfile)) {
...
function nfs_storage_display($nfs_storage_id) {
	...
	$table = new htmlobject_table_builder('nfs_lun_name', '', '', '', 'luns');
	$arHead = array();
