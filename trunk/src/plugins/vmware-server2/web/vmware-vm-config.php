...
<html lang="en">
<head>
	<title>VMWare Server 2 VM configuration</title>
...
if(htmlobject_request('action') != '') {
    switch (htmlobject_request('action')) {
        case 'update_cpus':
                ...
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 update_vm_cpus -n $vmware_vm_name -c $vmware_update_cpus -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                ...
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during update_cpus of VMWare Server2 vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Updated cpus on VMWare Server2 vm $vmware_vm_name<br>";
                }
        ...
        case 'update_ram':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 update_vm_ram -n $vmware_vm_name -r $vmware_update_ram -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during update_ram of VMWare Server2 vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Updated ram on VMWare Server2 vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'add_vm_net':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 add_vm_nic -n ".$vmware_vm_name." -x ".$vmware_nic_nr." -m ".$vmware_new_nic." -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during adding nic to VMWare Server2 vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Added network card to VMWare Server2 vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'remove_vm_net':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 remove_vm_nic -n ".$vmware_vm_name." -x ".$vmware_nic_nr." -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during removing nic of VMWare Server2 vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Removed network card from VMWare Server2 vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'update_vnc':
                ...
                if (!strlen($vmware_update_auth)) {
                    $strMsg .="Got emtpy VNC password. Skipping update of VMWare Server2 vm $vmware_vm_name<br>";
                    redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
                }
                if (!strlen($vmware_update_port)) {
                    $strMsg .="Got emtpy VNC port. Skipping update of VMWare Server2 vm $vmware_vm_name<br>";
                    redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
                }
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 update_vm_vnc -n $vmware_vm_name -va $vmware_update_auth -vp $vmware_update_port -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during update_vnc of VMWare Server2 vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Updated VNC config on VMWare Server2 vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'remove_vnc':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 remove_vm_vnc -n $vmware_vm_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during removing VNC config on VMWare Server2 vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Removed VNC config on VMWare Server2 vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'get_config':
            ...
            $vmware_server_resource_id = $vmware_server->id;
            $vmware_server_resource_ip = $vmware_server->ip;
            $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server2/bin/openqrm-vmware-server2 post_vm_config -n $vmware_vm_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
            // remove current stat file
            $statfile="vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
            if (file_exists($statfile)) {
                unlink($statfile);
            ...
            // and wait for the resulting statfile
            if (!wait_for_statfile($statfile)) {
                $strMsg = "Error refreshing config of VMWare Server2 vm $vmware_vm_name ! Please check the Event-Log<br>";
            } else {
                $strMsg = "";
            }
            redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
...
function vmware_vm_config() {
    ...
    $vmware_server_resource_ip = $vmware_server->ip;
    $vmware_server_resource_id = $vmware_server->id;
	$vmware_vm_conf_file=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server2/web/vmware-server2-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
    ...
    // backlink
    $backlink = "<a href='vmware-server2-manager.php?vmware_server_id=".$vmware_server_id."'>back</a>";
