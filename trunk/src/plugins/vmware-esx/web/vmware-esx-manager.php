			...
			case 'initialyze':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $id) {
						...
						$esx_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-esx/bin/openqrm-vmware-esx init -i $esx_ip -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$statfile="vmware-esx-stat/".$esx_ip.".vm_list";
						if (file_exists($statfile)) {
							unlink($statfile);
