			...
			case 'start':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						// and wait for the resulting statfile
						if (!wait_for_statfile($statfile, false)) {
							$strMsg .= "无法启动 $kvm_server_name ！请检查事件列表。<br>";
						} else {
							$strMsg .="启动 $kvm_server_name 。<br>";
						}
					}
					redirect($strMsg, "tab0");
				} else {
			...
			case 'restart':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						// and wait for the resulting statfile
						if (!wait_for_statfile($statfile, false)) {
							$strMsg .= "无法重启 $kvm_server_name ！请检查事件列表。<br>";
						} else {
							$strMsg .="重启 $kvm_server_name 。<br>";
						}
					}
					redirect($strMsg, "tab0");
				} else {
			...
			case 'delete':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						if ($remove_error == 1) {
							$strMsg .= "资源编号为 ".$kvm_vm_id." 的虚拟机正在被如下器件所使用：".$resource_is_used_by_appliance." 。<br>";
							$strMsg .= "资源编号为 ".$kvm_vm_id." 的虚拟机没有被删除!<br>";
							continue;
						}
						// remove vm
						$kvm_appliance = new appliance();
						...
						// and wait for the resulting statfile
						if (!wait_for_statfile($statfile, false)) {
							$strMsg .= "无法删除 $kvm_server_name ! 请检查事件列表。<br>";
						} else {
							$strMsg .="删除 $kvm_server_name 以及相关资源 $kvm_vm_id 。<br>";
						}
					}
					redirect($strMsg, "tab0");
				} else {
				...
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						if (!wait_for_statfile($statfile, true)) {
							$strMsg .= "无法将KVM虚拟机 $kvm_server_name 迁移到 $kvm_destination_host_resource_id ！请检查事件列表。<br>";
							continue;
						} else {
...
function kvm_server_select() {
	...
	$arHead['appliance_resource_id']['title'] ='资源编号';
	$arHead['appliance_resource_id']['sortable'] = false;

	$arHead['appliance_resource_ip'] = array();
	$arHead['appliance_resource_ip']['title'] ='IP地址';
	...
	if(count($arBody) > 0) {
		...
		$box->label = '<br><nobr><b>尚未创建任何虚拟化器件。</b></nobr>';
		$box_content = '<br><br><br><br>您需要先创建一个 '.$virtualization->name.' 器件！<br>';
		$box_content .= '<a href="/openqrm/base/server/appliance/appliance-new.php?currenttab=tab1"><b>创建器件</b></a><br>';
...
function kvm_server_display($appliance_id) {
	...
	$arHead['kvm_server_state']['title'] ='状态';

	$arHead['kvm_server_icon'] = array();
	$arHead['kvm_server_icon']['title'] ='操作系统类型';

	$arHead['kvm_server_id'] = array();
	$arHead['kvm_server_id']['title'] ='编号';

	$arHead['kvm_server_name'] = array();
	$arHead['kvm_server_name']['title'] ='名称';

	$arHead['kvm_server_resource_id'] = array();
	$arHead['kvm_server_resource_id']['title'] ='资源编号';

	$arHead['kvm_server_resource_ip'] = array();
	$arHead['kvm_server_resource_ip']['title'] ='IP地址';
	...
	$arHead1['kvm_vm_state']['title'] ='状态';
	$arHead1['kvm_vm_state']['sortable'] = false;

	$arHead1['kvm_vm_id'] = array();
	$arHead1['kvm_vm_id']['title'] ='资源编号';

	$arHead1['kvm_vm_name'] = array();
	$arHead1['kvm_vm_name']['title'] ='名称';

	$arHead1['kvm_vm_cpus'] = array();
	$arHead1['kvm_vm_cpus']['title'] ='处理器';

	$arHead1['kvm_vm_memory'] = array();
	$arHead1['kvm_vm_memory']['title'] ='内存';

	$arHead1['kvm_vm_ip'] = array();
	$arHead1['kvm_vm_ip']['title'] ='IP地址';

	$arHead1['kvm_vm_mac'] = array();
	$arHead1['kvm_vm_mac']['title'] ='MAC地址';

	$arHead1['kvm_vm_actions'] = array();
	$arHead1['kvm_vm_actions']['title'] ='操作';
			...
			if (!strstr($kvm_vm, "#")) {
				...
				if (!strcmp($kvm_vm_state, "1")) {
					...
					if ($remote_console_login_enabled) {
						...
						$vm_actions .= "<br>迁移到：".$migration_select;
						// end nobr
						$vm_actions .= "</nobr>";
					}
...
if(htmlobject_request('action') != '') {
	if (isset($_REQUEST['identifier'])) {
		switch (htmlobject_request('action')) {
			case 'select':
				foreach($_REQUEST['identifier'] as $id) {
					$output[] = array('label' => 'KVM VM管理器', 'value' => kvm_server_display($id));
				}
				break;
			case 'reload':
				foreach($_REQUEST['identifier'] as $id) {
		...
		$output[] = array('label' => 'KVM VM管理器', 'value' => kvm_server_select());
	}
} else if (strlen($kvm_server_id)) {
	$output[] = array('label' => 'KVM VM管理器', 'value' => kvm_server_display($kvm_server_id));
} else  {
	$output[] = array('label' => 'KVM VM管理器', 'value' => kvm_server_select());
}
