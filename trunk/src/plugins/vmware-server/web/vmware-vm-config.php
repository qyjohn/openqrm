	...
	if ($OPENQRM_USER->role == "administrator") {
		...
		switch (htmlobject_request('action')) {
			case 'update_cpus':
					...
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server update_vm_cpus -n $vmware_vm_name -c $vmware_update_cpus -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
			...
			case 'update_ram':
					...
					$vmware_server_resource_id = $vmware_server->id;
					$vmware_server_resource_ip = $vmware_server->ip;
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server update_vm_ram -n $vmware_vm_name -r $vmware_update_ram -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
					if (file_exists($statfile)) {
						unlink($statfile);
			...
			case 'add_vm_net':
					...
					$vmware_server_resource_id = $vmware_server->id;
					$vmware_server_resource_ip = $vmware_server->ip;
					$resource_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server/bin/openqrm-vmware-server add_vm_nic -n ".$vmware_vm_name." -x ".$vmware_nic_nr." -m ".$vmware_new_nic." -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
					if (file_exists($statfile)) {
						unlink($statfile);
			...
			case 'remove_vm_net':
					...
					$vmware_server_resource_id = $vmware_server->id;
					$vmware_server_resource_ip = $vmware_server->ip;
					$resource_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server/bin/openqrm-vmware-server remove_vm_nic -n ".$vmware_vm_name." -x ".$vmware_nic_nr." -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
					if (file_exists($statfile)) {
						unlink($statfile);
			...
			case 'update_vnc':
					...
					$vmware_server_resource_id = $vmware_server->id;
					$vmware_server_resource_ip = $vmware_server->ip;
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server update_vm_vnc -n $vmware_vm_name -va $vmware_update_auth -vp $vmware_update_port -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
					if (file_exists($statfile)) {
						unlink($statfile);
			...
			case 'remove_vnc':
					...
					$vmware_server_resource_id = $vmware_server->id;
					$vmware_server_resource_ip = $vmware_server->ip;
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server remove_vm_vnc -n $vmware_vm_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
					if (file_exists($statfile)) {
						unlink($statfile);
			...
			case 'get_config':
				...
				$vmware_server_resource_id = $vmware_server->id;
				$vmware_server_resource_ip = $vmware_server->ip;
				$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server post_vm_config -n $vmware_vm_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				// remove current stat file
				$statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
				if (file_exists($statfile)) {
					unlink($statfile);
